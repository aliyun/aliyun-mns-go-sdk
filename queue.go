...
func NewMNSQueue(name string, client MNSClient, qps int) AliMNSQueue {
	if name == "" {
	...
	queue.qpsMonitor = ratelimit.New(qps)
	return queue
}

func (p *MNSQueue) QPSMonitor() ratelimit.Limiter {
	return p.qpsMonitor
}
